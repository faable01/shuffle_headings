try:
  ...
  def swapNoun(target, candidate_list_to_replace):
    ...
    # 分かち書きに対応した品詞リスト
    part_of_speech_list = [token.part_of_speech for token in t.tokenize(target)]
    # 名詞,一般のインデックスを取得
    target_index_list = [i for i, x in enumerate(part_of_speech_list) if x.startswith('名詞,一般')]
    # 名詞,一般を、事前に取得した名詞,一般リストの中からランダムに選んだものと入れ替える
    for i in target_index_list:
      wakati[i] = len(candidate_list_to_replace) and random.choice(candidate_list_to_replace) or print('[candidate_list_to_replace()] is nothing.')
    # 入れ替えた結果を返却する
    return "".join(wakati)
  
  # Googleで検索する
  #（キーワードとstart位置を指定してGoogle検索結果のURLのタグのリストを返却する関数）
  def get_url_tag_list(keyword, start):
    url = 'https://www.google.co.jp/search'
    headers = {"User-Agent": "Mozilla/5.0 (Linux; U; Android 4.1.2; ja-jp; SC-06D Build/JZO54K) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30"}
  ...
  def search_by_yahoo(keyword, start):
    ...
    headers = {"User-Agent": "Mozilla/5.0 (Linux; U; Android 4.1.2; ja-jp; SC-06D Build/JZO54K) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30"}
    search_params = {"ei": "UTF-8", "fr": "top_smf", "meta": "vc=", "p": keyword, "b": start or 1}
    p_str = "&".join("%s=%s" % (k,v) for k,v in search_params.items())
    search_res = requests.get(url, params=p_str, headers=headers)
    search_soup = BeautifulSoup(search_res.text, 'html.parser')
  ...
  def exclude_specific_domains(url_tag_list, target_list):
    ...
    result_list = [url_tag for url_tag in url_tag_list if not re.match(f'{target}.*?', url_tag['href'])]
  ...
  def get_url_tag_list_up_to_specified_number(keyword, number, exclusion_target_list):
    ...
    while len(result_list) < number:
      ...
      url_tag_list_excluding_target_domain = exclusion_target_list and exclude_specific_domains(url_tag_list, exclusion_target_list) or url_tag_list
  ...
  for i, u in enumerate(urlList):
    ...
    # 取得したURLにアクセス
    h = {"User-Agent": "Mozilla/5.0 (Linux; U; Android 4.1.2; ja-jp; SC-06D Build/JZO54K) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30"}
  ...
  while True:
    ...
    for i in range(num_h2):
      ...
      base_h2 = len(all['h2']) and random.choice(all['h2']) or print("取得したh2見出しがありません")
      ...
      for j in range(num_h3):
        ...
        base_h3 = len(all['h3']) and random.choice(all['h3']) or print("取得したh3見出しがありません")
        ...
        for k in range(num_h4):
          ...
          base_h4 = len(all['h4']) and random.choice(all['h4']) or print("取得したh4見出しがありません")
